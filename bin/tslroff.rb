...
loop do
  ...
  when '-odir' then outdir = args.next
    ...
    filelist += if File.lstat(arg).directory?
                  Dir.glob("#{Regexp.escape(arg)}/**").select { |f| File.symlink?(f) or File.file?(f) }
                else
                  [arg]
                end
  end
...
loop do
  ...
  if src.symlink?
    ...
    pid = fork do
      ...
      require 'erb'
      File.open("#{odir.call}/#{src.output_filename}.html", File::CREAT | File::TRUNC | File::WRONLY, 0o644) do |ofile|
        ofile.write(ERB.new(template, trim_mode: '-').result(loopcontext))
      end
      exit
    end
    Process.detach(pid)

module Eqn
  ...
  def eqn_sqrt(parse_tree)
    rad = ExtendedRadical.new(font: @current_block.terminal_font.dup, style: @current_block.terminal_text_style.dup)
    gen_eqn [parse_tree.shift], output: rad
    @current_block << rad
  end

  def eqn_over(parse_tree)
    frac = Fraction.new(numerator: Numerator.new(font: @current_block.terminal_font.dup, style: @current_block.terminal_text_style.dup),
                        denominator: Denominator.new(font: @current_block.terminal_font.dup, style: @current_block.terminal_text_style.dup),
                        font: @current_block.terminal_font.dup, style: @current_block.terminal_text_style.dup)
  ...
  def eqn_script(parse_tree)
    ...
    s = obj.new(font: @current_block.terminal_font.dup, style: @current_block.terminal_text_style.dup)
    ...
    @current_block.text[-2].style.css[:width] = 0 if [SuperScript, SubScript].include? @current_block.text[-2].class and @current_block.terminal_text_obj.empty?
    @current_block << s
  end

  def eqn_bounds(parse_tree)
    bound = EqnBlock.new(font: @current_block.terminal_font.dup, style: @current_block.terminal_text_style.dup)

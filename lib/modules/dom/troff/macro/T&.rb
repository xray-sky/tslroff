...
module Troff
  ...
  def format_row
    ...
    # I'm a bit confused when font/size resets/doesn't reset
    ...
    loop do
      ...
      until fmt.empty? do
        ...
        when /^(f.[A-Z]?)/ # REVIEW why were we accepting two digits? a font position I think is only one.
          ...
          #@current_block = cell
          unescape @state[:escape_char] + Regexp.last_match[1], output: cell
        ...
        when /^(_|=)/
          ...
          row_adj.text.last.style.css[:line_height] = '49%'
